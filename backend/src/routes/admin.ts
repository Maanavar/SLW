  ...
  customers: z
    .array(
      z.object({
        ...
        hasBillNo: z.boolean().optional().default(false),
  ...
  jobs: z
    .array(
      z.object({
        ...
        jobFlowType: z.enum(['slw_work', 'agent_work']).optional(),
        externalDc: z.boolean().optional(),
        agentName: z.string().trim().max(120).optional(),
        agentCommissionAmount: z.number().min(0).optional(),
        agentTdsAmount: z.number().min(0).optional(),
        agentSettlementPaidAmount: z.number().min(0).optional(),
      ...
      if (payload.jobs.length > 0) {
        await tx.job.createMany({
          data: payload.jobs.map((job) => ({
            ...
            billNo: job.billNo ?? null,
            dcNo: job.dcNo ?? null,
            vehicleNo: job.vehicleNo ?? null,
            dcDate: job.dcDate ?? null,
            dcApproval: job.dcApproval ?? null,
            jobFlowType: job.jobFlowType ?? null,
            externalDc: job.externalDc ?? false,
            agentName: job.agentName ?? null,
            agentCommissionAmount: job.agentCommissionAmount ?? null,
            agentTdsAmount: job.agentTdsAmount ?? null,
            agentSettlementPaidAmount: job.agentSettlementPaidAmount ?? null,
