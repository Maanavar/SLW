  ...
  if (!localStorage.getItem(STORAGE_KEYS.WORK_TYPES)) {
    const defaultWorkTypes = [
      ...
      },
      {
        id: 4,
        category: 'Skimming',
        name: 'Flywheel Preplate Throwing',
        shortCode: '',
        defaultUnit: 'Per piece',
        defaultRate: 0,
      },
      ...
      },
      {
        ...
        name: 'Stud',
        shortCode: '',
        defaultUnit: 'Per stud',
        defaultRate: 100,
      },
      {
        id: 46,
        category: 'Bolt',
        name: 'Thread Sleeve M6',
        shortCode: 'M6',
        defaultUnit: 'Per sleeve',
        defaultRate: 150,
      },
      {
        id: 47,
        category: 'Bolt',
        name: 'Thread Sleeve M8',
        shortCode: 'M8',
        defaultUnit: 'Per sleeve',
        defaultRate: 200,
      },
...
function setupSearchableDropdown(inputId, dropdownId, items, onSelect, renderItem, getSearchTerms) {
  ...
  function renderDropdown(filteredItems) {
    if (filteredItems.length === 0) {
      dropdown.innerHTML =
        '<div style="padding: var(--space-lg); text-align: center; color: var(--text-muted);">No results found</div>';
      return;
    }
      ...
      const dcBadge =
        dcStatus === 'Completed'
          ...
          : dcStatus === 'Pending DC'
            ...
            : '';
      const actionButton =
        !job.jobCardLine || job.jobCardLine === 1
...
function syncCurrentWorkTypeSelection(updatedWorkType) {
  document.querySelectorAll('.job-line').forEach((row) => {
    if (!row._selectedWorkType || row._selectedWorkType.id !== updatedWorkType.id) {
return;
}
    ...
    if (search) {
search.value = updatedWorkType.name;
}
    if (hidden) {
hidden.value = updatedWorkType.id;
}
    updateJobLineSuggestion(lineId);
  });
  updateJobReview();
...
function cascadeCustomerNameChange(customerId, customerName) {
  const jobs = getJobs().map((job) =>
    job.customerId === customerId ? { ...job, customerName } : job
  );
  const payments = getPayments().map((payment) =>
    payment.customerId === customerId ? { ...payment, customerName } : payment
  );
...
function cascadeWorkTypeNameChange(workTypeId, workTypeName) {
  const jobs = getJobs().map((job) =>
    job.workTypeId === workTypeId ? { ...job, workTypeName, workName: workTypeName } : job
  );
...
function openCustomerModal(customerId = null) {
  ...
  document.getElementById('customerModalTitle').textContent = current
    ? 'Edit Customer'
    : 'Add Customer';
  ...
  document.getElementById('customerHasCommission').checked = current
    ? !!current.hasCommission
    : false;
...
function openWorkTypeModal(workTypeId = null) {
  ...
  document.getElementById('workTypeModalTitle').textContent = current
    ? 'Edit Work Type'
    : 'Add Work Type';
...
return;
  ...
  updatePendingBadge();
  showToast(
    active ? 'Customer Restored' : 'Customer Deleted',
    `${customer.name} ${active ? 'restored' : 'deactivated'}`
  );
}
...
return;
  ...
  refreshJobReports();
  showToast(
    active ? 'Work Type Restored' : 'Work Type Deleted',
    `${workType.name} ${active ? 'restored' : 'deactivated'}`
  );
}
  ...
  if (paymentExportExcel) {
    paymentExportExcel.addEventListener('click', () => {
      ...
      payments
        ...
        .forEach((p) => {
          const coverageLabel =
            p.paymentCoverageType === 'single'
              ? p.paymentForDate
              : p.paymentCoverageType === 'range'
                ? `${p.paymentForFromDate} to ${p.paymentForToDate}`
                : p.paymentCoverageType === 'month'
                  ? p.paymentForMonth
                  : '-';
          rows.push([
            p.date,
...
return;
  ...
  if (customerListTable) {
    customerListTable.addEventListener('click', (e) => {
      ...
      if (btn) {
editCustomer(parseInt(btn.dataset.editCustomer, 10));
}
...
return;
  ...
  if (workTypeListTable) {
    workTypeListTable.addEventListener('click', (e) => {
      ...
      if (btn) {
editWorkType(parseInt(btn.dataset.editWorktype, 10));
}
      ...
      const dcBadge =
        dcStatus === 'Completed'
          ...
          : dcStatus === 'Pending DC'
            ? '<span class="badge badge-orange">DC Pending</span>'
            : '';
      const lineList = card.jobs
        .map((line) => `${line.workTypeName || line.workName || 'Job'} x ${line.quantity}`)
        .join(' · ');
...
filtered = filtered.filter((j) => j.date <= toDate);
  ...
  if (customerId) {
filtered = filtered.filter((j) => j.customerId === parseInt(customerId, 10));
}
